...
class OKXClient:
    def __init__(self, base_url: str = "https://www.okx.com", api_key: str = "", secret_key: str = "", passphrase: str = "", demo: bool = False, timeout: float = 15.0, retry_attempts: int = 3, session=None, timestamp: Optional[Callable[[], str]] = None, sleep: Callable[[float], None] = time.sleep):
        parsed_url = urlparse(base_url)
        if parsed_url.scheme != "https" or parsed_url.hostname not in {"www.okx.com", "my.okx.com", "app.okx.com"}:
    ...
    def _private_headers(self, timestamp: str, method: str, request_path: str, body: str) -> dict:
        ...
        signature = base64.b64encode(hmac.new(self.secret_key.encode(), prehash, hashlib.sha256).digest()).decode()
        headers = {"Content-Type": "application/json", "OK-ACCESS-KEY": self.api_key, "OK-ACCESS-SIGN": signature, "OK-ACCESS-TIMESTAMP": timestamp, "OK-ACCESS-PASSPHRASE": self.passphrase}
        if self.demo:
    ...
    def _validate_payload(payload: dict) -> list:
        ...
        for item in data:
            if isinstance(item, dict) and str(item.get("sCode", "0")) not in {"", "0"}:
                raise OKXAPIError(f"OKX order error {item.get('sCode')}: {item.get('sMsg', '')}")
        return data

    def _request(self, method: str, path: str, params: Optional[dict] = None, body=None, private: bool = False) -> list:
        method = method.upper()
        ...
        for attempt in range(self.retry_attempts):
            ...
            headers = self._private_headers(timestamp, method, request_path, body_text) if private else {"Content-Type": "application/json"}
            try:
                response = self.session.request(method=method, url=self.base_url + request_path, headers=headers, data=body_text or None, timeout=self.timeout)
                ...
                last_error = exc
                transient = isinstance(exc, requests.RequestException) or (isinstance(exc, OKXAPIError) and ("HTTP 429" in str(exc) or "HTTP 5" in str(exc)))
